...
// Valid task priority values; new tasks default to the lowest priority (P3)
const TASK_PRIORITIES = ['P1', 'P2', 'P3'];
const DEFAULT_TASK_PRIORITY = 'P3';
...
app.post('/api/tasks', (req, res) => {
  try {
    ...
    let taskPriority = DEFAULT_TASK_PRIORITY;
...
app.put('/api/tasks/:id', (req, res) => {
  try {
    ...
    let taskPriority = existingTask.priority;
    if (priority !== undefined) {
      if (!TASK_PRIORITIES.includes(priority)) {
        return res.status(400).json({ error: 'Priority must be one of P1, P2, P3' });
      }
...
app.patch('/api/tasks/:id', (req, res) => {
  try {
    ...
    const stmt = db.prepare(`UPDATE tasks SET ${setClauses.join(', ')} WHERE id = @id`);
